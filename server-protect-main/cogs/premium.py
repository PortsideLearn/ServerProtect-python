...
class Premium(commands.Cog):
    ...
    async def plus(self, ctx):
        ...
        embed = discord.Embed()
        embed.title = '⭐ | Немного о Server Protect Premium!'
        embed.description = f'''
...
Деньги пойдут на оплату хостинга для бота, чтобы он продолжал защищать ваш прекрасный сервер.
        ...
        embed.add_field(name='Что даёт Server Protect Premium?', value='''
...
- Роль "Premium Обладатель" на дискорд сервере бота. (Роль выдается только создателю сервера)
        ...
        row = ActionRow(
            Button(
                ...
                label="Купить Server Protect Premium для этого сервера",
                custom_id="buy"
            )
        )
        ...
        async def on_bill_click(inter):
            ...
            if messages.has_premium(inter.guild.id):
                return await messages.err(ctx, "На этом сервере уже активирован Server Protect Premium.")
            ...
            try:
                ...
                comment = f"Покупка Server Protect Premium для сервера {inter.guild.name} (ID: {inter.guild.id})"
                ...
                embed3.description += "Server Protect Premium активируется в течение двух минут с момента оплаты."
