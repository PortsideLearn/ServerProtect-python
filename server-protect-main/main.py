...
async def on_ready():
    ...
    await bot.get_channel(1028344322292203710).send(embed=discord.Embed(title = 'Бот в сети!', description = f"Запуск произошёл успешно!\nВремя запуска: {word.hms2(time.time() - start)}\nДата и время: {time4logs()}", color = 0x00FF00))
    await bot.change_presence(status = discord.Status.idle, activity = discord.Game("sp.help | Бот запущен!"))
...
**ID:** {guild.id}
                ...
                await bot.get_channel(1028344322292203706).send(embed=lb)
    else:
