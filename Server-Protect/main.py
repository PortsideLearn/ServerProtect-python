...
async def on_ready():
    ...
    await bot.get_channel(1028412269668876300).send(embed=discord.Embed(title = 'Бот в сети!', description = f"Запуск произошёл успешно!\nВремя запуска: {word.hms2(time.time() - start)}\nДата и время: {time4logs()}", color = 0x00FF00))
    await bot.change_presence(status = discord.Status.idle, activity = discord.Game("Recoding full Source (15%/100%)"))
...
`2.` Убедись, что у меня есть права администратора для работы.
            ...
            row = ActionRow(
                Button(
                    ...
                    label="Поддержка",
                    emoji="❔",
                    url="https://discord.gg/Nx5YYFFS5J"
                )#№,
                #Button(
                    #style=ButtonStyle.link,
                    #label="Документация",
            ...
            if bot.user.id == 1008694483321372774:
...
**ID:** {guild.id}
                ...
                await bot.get_channel(1028412134612279378).send(embed=lb)
    else:
...
@bot.command()
async def ram(ctx):
    if ctx.author.id in [1012347822760271912]:
        emb = discord.Embed()
        emb.color = 0xffffff
...
@app.route('/')
def main():
    return jsonify(users=len(client.users), guilds=len(client.guilds), ping=int(client.latency * 1000))

def run():
    app.run(host='0.0.0.0', port=1655)
