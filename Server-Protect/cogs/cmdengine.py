...
`{ctx.prefix}discrim` – поиск участников с определённым Discord тегом
            """)
            cpplus = discord.Embed(color=Color.primary, title="⭐ | Список команд: Управление подпиской Server Protect Premium", description=f"""
...
`{ctx.prefix}plus` – подробнее о подписке
            ...
            selectmenu = SelectMenu(
                ...
                options=[
                    ...
                    SelectOption("SP Premium", "Управление подпиской Server Protect Premium", emoji="⭐")
                ]
            )
            button = ActionRow(
                Button(
...
**Не используйте скобочки при указании параметров**
    ...
    async def invite(self, ctx):
        ...
        embed.description = '''
[🤖 Пригласить бота](https://discord.com/oauth2/authorize?client_id=1008694483321372774&scope=applications.commands%20bot&permissions=8)
[❔ Поддержка](https://discord.gg/Nx5YYFFS5J)
        '''
        await ctx.send(embed=embed)
...
> Дата создания сервера: <t:{int(dt.timestamp())}> (<t:{int(dt.timestamp())}:R>)
        ...
        if messages.has_premium(ctx.guild.id):
            embed.add_field(name="Server Protect Premium активен!", value="Спасибо за поддержку Server Protect :heart:", inline=False)
    ...
    async def prefix(self, ctx, prefix):
        if prefix.lower() in ["reset", "сброс", "sp."]:
...
👥 Пользователей: **{len(self.bot.users)}**
        ...
        embed.add_field(
            ...
            inline=False,
            value=f'''
📆 Дата создания: **16 августа 2022 года**
🐍 Версия Python: **3.10**
📄 Версия: **1.0 (16 августа 2022 года)**
👨‍💻 Разработчики: **youp1xxx#1416**
            '''
        )
        embed.add_field(
            name="Ссылки",
            inline=False,
            value=f'''
[Пригласить бота](https://discord.com/oauth2/authorize?client_id=1028334935485980702&scope=applications.commands%20bot&permissions=8)
[Сервер поддержки](https://discord.gg/yNJaynAMKe)
[Сайт бота](https://youtu.be/dQw4w9WgXcQ)
        ...
        embed.add_field(
            ...
            inline=False,
            value=f'''
<:cymonIMG:1028405864547242095> **Cymon#4380** - за оригинальный код бота и аватарку боту. Жалко краш протекта :'(
<:artembayIMG:1028407023466979388> **Artem Bay#0547** - за экстренную помощь по хостингу
            '''
        )
        embed.set_footer(text="© 2022, youp1xxx | Все права защищены ботом Server Protect")
